...
class Uniform15KPC(Dataset):
    def __init__(self, root_dir, subdirs, tr_sample_size=10000,
        ...
        self.subdirs = subdirs  # category ids in ShapeNet
        self.scale = scale
        ...
        for cate_idx, subd in enumerate(self.subdirs):
            ...
            sub_path = os.path.join(root_dir, subd, self.split) # e.g., /../data/ShapeNetCore.v2.PC15K/03001627/train/
            ...
            for mid in all_mids:
                # e.g., /../data/ShapeNetCore.v2.PC15K/03001627/train/xxx.npy
                ...
                assert point_cloud.shape[0] == 15000    # contains 15,000 sampled points for each shape.
        ...
        random.Random(38383).shuffle(self.shuffle_idx)  # random.shuffle() changes the x list in place
        self.cate_idx_lst = [self.cate_idx_lst[i] for i in self.shuffle_idx]
        # self.all_points is a list. len(self.all_points) = N, and each element is a ndarray of shape (1, 15000, 3)
        self.all_points = [self.all_points[i] for i in self.shuffle_idx]
        self.all_cate_mids = [self.all_cate_mids[i] for i in self.shuffle_idx]

        # Normalization
        self.all_points = np.concatenate(self.all_points)  # Now, self.all_points is a ndarray of shape (N, 15000, 3)
    ...
    def __getitem__(self, idx):
        ...
        # modify: coordinates are normalized to the range [-1,1]
        # tr_out = pc_normalize(tr_out)
        # te_out = pc_normalize(te_out)
...
class ShapeNet15kPointClouds(Uniform15KPC):
    def __init__(self, root_dir="data/ShapeNetCore.v2.PC15k",
        ...
        self.tr_sample_size = tr_sample_size    # number of points
        self.te_sample_size = te_sample_size
        self.cates = categories
        # convert category names (str) to corresponding ids in ShapeNet
