...
class GaussianDiffusion:
    ...
    def p_sample_loop(self, denoise_fn, shape, device,
        ...
        img_t = noise_fn(size=shape, dtype=torch.float, device=device)  # start with Gaussian noise
        ...
        # img_t = self.q_sample(x_start=data_start, t=t, noise=noise)   # start with GroundTruth + Gaussian noise
    ...
    def p_sample_loop_v2(self, denoise_fn, data_start, shape, device,
                         noise_fn=torch.randn, clip_denoised=True, keep_running=False):
        """
        Generate samples
        keep_running: True if we run 2 x num_timesteps, False if we just run num_timesteps
    ...
    def p_sample_loop_trajectory_v2(self, denoise_fn, data_start, shape, device, freq,
                                    noise_fn=torch.randn,clip_denoised=True, keep_running=False):
        """
        Generate samples, returning intermediate images
        Useful for visualizing how denoised images evolve over time
...
class Model(nn.Module):
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        self.model = PointTransformerSeg42(in_channels=args.model_input_dim, num_classes=args.nc, embed_dim=args.embed_dim,
                                           stride=args.stride, nsample=args.nsample, global_pooling=args.global_pooling)
    ...
    def _denoise(self, data, t):
        ...
        # out = self.model(data.transpose(1, 2)).transpose(1, 2) # test without t
        out = self.model(data, t).transpose(1, 2)
    ...
    def gen_samples_v2(self, shape, data_start, device, noise_fn=torch.randn,
                       clip_denoised=True,
                       keep_running=False):
        return self.diffusion.p_sample_loop_v2(self._denoise, shape=shape, data_start=data_start,
                                               device=device, noise_fn=noise_fn,
                                               clip_denoised=clip_denoised,
                                               keep_running=keep_running)

    def gen_sample_traj_v2(self, shape, data_start, device, freq, noise_fn=torch.randn,
                           clip_denoised=True,keep_running=False):
        return self.diffusion.p_sample_loop_trajectory_v2(self._denoise, shape=shape, data_start=data_start,
...
def train(gpu, opt, output_dir, noises_init):
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        # lr_scheduler.step()
        # logger.info('current learning rate: {:>.8f}'.format(lr_scheduler.get_last_lr()[0]))
        with warmup_scheduler.dampening():
            if epoch >= opt.warmup_period - 1:
                lr_scheduler.step()
        ...
        for i, data in enumerate(dataloader):
            ...
            optimizer.zero_grad()
            loss.backward()
            # TODO: there is an error when getGradNorm(model)
        ...
        # Modify: comment diagnosis and visualization to accelerate training
        ...
        #         # start with GroundTruth + Gaussian noise
        #         # x_gen_eval = model.gen_samples_v2(new_x_chain(x, 4).shape, x[:4],
        #         #                                   x.device, clip_denoised=False)
        #         # x_gen_list = model.gen_sample_traj_v2(new_x_chain(x, 1).shape, x[:1],
        #         #                                       x.device, freq=40, clip_denoised=False)
...
def main():
    ...
    if opt.category == 'airplane':
        ...
        opt.lr_gamma = 0.997
        opt.warmup_period = 10
    elif opt.category == 'car':
        opt.lr_gamma = 0.998
        opt.warmup_period = 5
    ...
    # copy_source('model/point_transformer/point_transformer_seg.py', output_dir)
    # copy_source('model/point_transformer/point_transformer_blocks.py', output_dir)
...
def parse_args():
    ...
    # Modify: learning hyperparameters
    parser.add_argument('--lr', type=float, default=2e-4, help='learning rate for E, default=0.0002')
    parser.add_argument('--beta1', type=float, default=0.5, help='beta1 for adam. default=0.5')
    parser.add_argument('--decay', type=float, default=0, help='weight decay for EBM')
    parser.add_argument('--grad_clip', type=float, default=None, help='weight decay for EBM')
    parser.add_argument('--lr_gamma', type=float, default=0.996, help='lr decay for EBM')
    ...
    parser.add_argument('--nsample', type=json.loads, default="[16, 32, 32, 32, 32]")
    ...
    '''eval'''
    parser.add_argument('--saveIter', default=50, type=int, help='unit: epoch')
